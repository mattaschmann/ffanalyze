...
def sheet(position: Position) -> pd.DataFrame:
    ...
    match position:  # total opp
        ...
        case Position.WR | Position.RB | Position.TE:
            ...
            result[col_names.opp_col] = pos_df[["RuAt", "Tar"]].sum(axis=1) + (
                pos_df["RetY"] / RET_YDS_PT
            )
        case Position.K:
            col_names = ColNames("O")
            result[col_names.opp_col] = pos_df[
                ["Pat", "0-19", "20-29", "30-39", "40-49", "50+"]
            ].sum(axis=1)
        case Position.DEF:
            col_names = ColNames("O")
            result[col_names.opp_col] = pos_df[
    ...
    result[col_names.pt_opp_z_col] = math_utils.z_score(
        result[col_names.pt_opp_col]
    )  # Points per opp z-score
    result[col_names.ops_g_z_col] = math_utils.z_score(
